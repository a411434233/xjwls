 ...
 * @param {*} url  请求地址
 * @param {*} data  参数
 * @param {*} user  是否需要token
 * @param {}
 */

function Post(url, data, user) {
  return HttpRequest(url, 'POST', data, user);
}
function Get(url, data, user) {
  return HttpRequest(url, 'GET', data, user);
}
function HttpRequest(url, method, data, user = false) {

import { Post, Get, api, Token, ShowNoneToast, NavigateTo, SwitchTab, GetChannelCode, SetStorage, Product, getSysTemInfo, Debounce, ShowLoading, HideLoading } from '../../../utils/common';
const app = getApp();
Page({
  data: {
    shopModel: false,
  ...
  getproInfo(productId) {
    Post(api.GetProductInfo2 + '?productId=' + productId).then(res => {
      if (res.data.Code == -1) return ShowNoneToast(res.data.Msg);
  ...
  getProductTaste(productId) {
    Post(api.GetProductTaste4 + '?productId=' + productId).then(res => {
      ...
      this.getguige(info.specifications);
      this.setData(info);
    });
  },
  // 推荐商品
  ...
  async initGocoup(discountId) {
    ...
    let [url, data, that] = [api.GetCoupons, { Token: info.token, DiscountId: discountId, channelCode: channelCode }, this];
  ...
  addShopCat(e) {
    ...
    Post(api.PostShopCart, data, true).then(res => {
      if (res.data.Code == 1) {
  ...
  setLabel(e) {
    ...
    specifications[index].item.forEach((res, index) => {
      res.isChecked = idx == index;
    });
    this.getguige(specifications);
  ...
  getguige(guige = []) {
    ...
    if (typeof guige == 'undefined') return {};
    let p = guige.map(res => {
      return res.item.filter(val => val.isChecked).pop();
    });
    let m = p.map(val => val.CategoryName).join('、');
    this.setData({ guige: m });
    return p.shift() || {};
  },
  //推荐商品详情页
  goodsDetail(e) {

import { Post, Get, api, ShowNoneToast } from '../../../utils/common';
const app = getApp();
Page({
  ...
  tj() {
    ...
    if (this.data.status == 1) {
      let data = { OrderId: this.data.orderid, AfterType: 1, RefundReason: this.data.yuanyin, RefundExplain: this.data.shuoming };
      ...
      let data = { OrderId: this.data.orderid, AfterType: 2, RefundReason: this.data.yuanyin, RefundExplain: this.data.shuoming, RefundShipId: this.data.wuliu, refundImagesView: this.data.imgUrls.join(',') };
      Post(api.PostReceiptApply, data).then(res => {
        if (res.data.Code == 1) {
          my.navigateTo({ url: '/pages/components/aftersaledetail/aftersaledetail?orderid=' + this.data.orderid });
        }

import { Post, Get, NavigateTo, GetStorage, JsonSort, api, Product } from '../../../utils/common';
const app = getApp();
Page({
  data: { left: 0, activeTab: 0, currIndex: 0, sortIndex: 0, screenWidth: 375, Categorys: [], productsLeft: [], subtabname: 'all', shopNum: '0', sort: true, className: '', price_title: '价格', productList: [] },
  async onLoad() {
  ...
  bPoint() {
    GetStorage('qdmc').then(res => {
      if (res.success == true) {
        ...
        let [url, data, that] = [api.PostStatisticSystem, { PageName: PageName, ClickPlace: ClickPlace, SoureChannel: SoureChannel }, this];
        Post(url, data, true);
      }
    });
  },

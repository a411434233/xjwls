import {api, Get, NavigateTo, Post, Product, ShowNoneToast} from '../../../utils/common';
...
Page({
    ...
    async del(e) {
        ...
        let res = await Post(api.DeleteShopCart, {shopId: e.currentTarget.dataset.id}, false, true);
        if (res.data.Code == 1) {
    ...
    async gobuyorder() {
        ...
        suclist.forEach(res => {
            ...
            if (shoppingInfo.length > 0) {
                orderlistobj.shoppingOrderView.push({
        ...
        app.orderdata = res.data.Data;
        my.navigateTo({url: '/pages/components/buyorder/buyorder'});
    },
    godetail(e) {
        let dt = e.currentTarget.dataset;
        let [sid, pid, skuId] = [dt.id, dt.pid, dt.skuId];
